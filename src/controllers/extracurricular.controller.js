...
extracurricularCtrl.consultarExtracurricular = async (req, res) => {
    try {
        ...
        const result = await extracurricularModel.findAll({
            ...
            include: {association: 'extracurricularAsTeacher'}
        });
        res.json({
            mensaje: 'ok',
            result
...
extracurricularCtrl.consultarId = async (req, res) => {
    try {
        const {id} = req.params;
        const result = await extracurricularModel.findOne({
            where: {id: id},
            include: {association: 'extracurricularAsTeacher'}
        });
...
extracurricularCtrl.crearExtracurricular = async (req, res) => {
    ...
    await extracurricularModel.create({
        imagen,
        activity,
        startDate,
        finalDate,
...
extracurricularCtrl.actualizarExtracurricular = async (req, res) => {
    ...
    try {
        ...
        await extracurricularModel.update({
            activity,
            startDate,
            finalDate,
            idTeacher,
...
extracurricularCtrl.misExtracurriculares = async (req, res) => {
    ...
    const inscripcion = await extracurricularInscriptionModel.findAll({
        where: {idEstudiante: idEstudiante, isActive: 1},
        include: {
            association: 'extracurricularInscriptionAsExtracurricular',
            include: {association: 'extracurricularAsTeacher'}
        }
    })
...
extracurricularCtrl.pago = async (req, res) => {
    ...
    const {monto, idExtracurricular, metodoPago, idEstudiante, isActive} = req.body
    ...
    if (idExtracurricular == null) {
        ...
        const inscripcion = await extracurricularInscriptionModel.findOne({
            where: {
                idExtracurricular: idExtracurricular,
                isActive: isActive
            }, include: {association: 'extracurricularInscriptionAsExtracurricular'}
        })
        if (inscripcion === null) {
            const datos = await extracurricularInscriptionModel.create({
                monto,
                idExtracurricular,
                metodoPago,
                idEstudiante,
            ...
            if (inscripcion.extracurricularInscriptionAsExtracurricular.finalDate > new Date()) {
                ...
                const datos = await technicalInscription.create({
                    monto,
                    idExtracurricular,
                    metodoPago,
                    idEstudiante,
